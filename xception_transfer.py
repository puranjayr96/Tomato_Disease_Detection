...
with tf.Session() as sess:
    ...
    bftx = h5py.File('bftx_xception.h5','w')
    bftx.create_dataset('bftx',data = bottleneck_features_train)
    bftx.close()
    with open('bfty_xception.pkl', 'wb') as f:
    ...
    bfvx = h5py.File('bfvx_xception.h5','w')
    bfvx.create_dataset('bfvx',data = bottleneck_features_validation)
    bfvx.close()
    with open('bfvy_xception.pkl', 'wb') as f:
...
def load_bottleneck_data(training_file, validation_file):
    ...
    h5f = h5py.File('bftx_xception.h5','r')
    X_train2 = h5f['bftx'][:]
    h5f.close()
    h5f = h5py.File('bfvx_xception.h5','r')
    X_val2 = h5f['bfvx'][:]
    h5f.close()
    with open('bfty_xception.pkl', 'rb') as f:
        y_train2 = pickle.load(f)
    with open('bfvy_xception.pkl', 'rb') as f:
...
with tf.Session() as sess:
    ...
    if ans == 'y':
        model.save_weights('xception_bottleneck_weights.h5')
        print("Saved")
