...
#     bftx = h5py.File('bftx_vgg_19.h5','w')
#     bftx.create_dataset('bftx',data = bottleneck_features_train)
#     bftx.close()
#     with open('bfty_vgg_19.pkl', 'wb') as f:
...
#     bfvx = h5py.File('bfvx_vgg_19.h5','w')
#     bfvx.create_dataset('bfvx',data = bottleneck_features_validation)
#     bfvx.close()
#     with open('bfvy_vgg_19.pkl', 'wb') as f:
...
#     h5f = h5py.File('bftx_vgg_19.h5','r')
#     X_train2 = h5f['bftx'][:]
#     h5f.close()
#     h5f = h5py.File('bfvx_vgg_19.h5','r')
#     X_val2 = h5f['bfvx'][:]
#     h5f.close()
#     with open('bfty_vgg_19.pkl', 'rb') as f:
#         y_train2 = pickle.load(f)
#     with open('bfvy_vgg_19.pkl', 'rb') as f:
...
res_model = keras.applications.VGG19(include_top=False,weights='imagenet',input_shape=(224,224,3))
...
with tf.Session() as sess:
    ...
    history =model.fit(X_train1, y_train1, epochs=epochs, batch_size=batch_size,
                       validation_data=(X_val1, y_val1), shuffle=True, verbose=1,callbacks=[metrics] )
    print(metrics.f1s)
    ...
    if ans == 'y':
        model.save_weights('vgg_19_bottleneck_weights.h5')
        print("Saved")
